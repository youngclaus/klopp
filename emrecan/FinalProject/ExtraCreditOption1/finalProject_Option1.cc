...
class Important {
public:
  int min, minT, max, maxT, avg, total;
...
int main() {
  ...
  while (storeClock < maxClockTime) {   //Open 17 hours after 8 am
    ...
    if (store.head != NULL) {     //should only run if something is there
      ...
      if (store.numCustomersInLine(store.head) > 1) {
        if (store.head->next->orderTime == 0) {
          cout << "-->Customer " << store.head->next->in << " left.\n";
          store.dequeue2();
        }
        else {
          store.head->next->orderTime--;
        }
      }
    }
  ...
  if (c < 1) {
    cout << "\nAvg peeps in Line 1:    Less than 1 person [" << c << "]";
  }
  else {
    cout << "\nAvg peeps in Line 1:    " << int(c) << " people";
  }
  cout << "\n-------------------------------------------------------------------------------------------\n";
}